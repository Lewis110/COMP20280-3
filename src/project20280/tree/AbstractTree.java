...
import src.project20280.interfaces.Tree;
...
public abstract class AbstractTree<E> implements Tree<E> {
    ...
    public int height_recursive(Position<E> p) {
        int h = 0;
        for (Position<E> c : children(p)) h = Math.max(h, 1 + height_recursive(c));
        return h;
    }
    ...
    public Iterable<Position<E>> breadthfirst() {
        ...
        java.util.LinkedList<Position<E>> fringe = new java.util.LinkedList<>();
        fringe.add(root());
        while (!fringe.isEmpty()) {
            Position<E> p = fringe.removeFirst();
            snapshot.add(p);
            for (Position<E> c : children(p)) fringe.addLast(c);
        }
        return snapshot;
    }
}