...
public class HeapPriorityQueue<K, V> extends AbstractPriorityQueue<K, V> {
    ...
    public HeapPriorityQueue(K[] keys, V[] values) {
        ...
        for (int i = 0; i < n; i++) heap.add(new PQEntry<>(keys[i], values[i]));
        heapify();
    }
    ...
    protected void downheap(int j) {
        while (hasLeft(j)) {
            ...
            if (hasRight(j)) {
                ...
                if (compare(heap.get(rightIndex), heap.get(leftIndex)) < 0) smallChildIndex = rightIndex;
            }
    ...
    public Entry<K, V> removeMin() {
        ...
        swap(0, heap.size() - 1);
        Entry<K, V> answer = heap.remove(heap.size() - 1);
